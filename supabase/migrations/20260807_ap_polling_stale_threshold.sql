-- Migration: Align ap-polling stale threshold with sparse 3x/day schedule
-- Created: 2026-08-07
-- Background:
--   ap-polling fires at 8/12/17 America/Denver only. The prior 25-minute
--   stale threshold (left over from a */15 era) permanently listed
--   ap-polling in ops_health_summary.stale_crons except for ~25 minutes
--   after each run — false red, especially after every bot restart.
--
--   Match the ap-polling-fresh heartbeat probe (maxAgeHrs=20): treat as
--   stale only when no run for 20 hours. Weekend Fri 17:00 → Mon 08:00
--   (~63h) still flags; external watchdogs already business-hours-gate.
...
-- High-frequency "always running" jobs + sparse AP (20h). Once-daily/weekly
-- jobs are NOT listed — they legitimately go 20+ hours between runs.
-- Names match defineJob({ name: ... }) in src/cron/jobs/index.ts (kebab-case).
cron_thresholds AS (
    SELECT *
    FROM (
        VALUES
            ('ap-polling',                INTERVAL '20 hours'),
...
COMMENT ON VIEW public.ops_health_summary IS
    'Ops control-plane health rollup. ap-polling stale threshold = 20h (sparse 8/12/17 Denver).';
