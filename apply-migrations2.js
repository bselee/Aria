...
async function main() {
    ...
    // Check ap_activity_log columns
    const actCols = await client.query(
        "SELECT column_name FROM information_schema.columns WHERE table_name = 'ap_activity_log' AND column_name IN ('notified_slack')"
    );
    console.log("Activity log columns:", actCols.rows.map(r => r.column_name));
