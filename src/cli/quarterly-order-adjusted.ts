...
// SUPERSESSION: S-4796 (22x14x6") replaces S-4905 (24x14x6")
// Combine their consumption and use S-4796 going forward
const superseded = {
    old: 'S-4905',
    new: 'S-4796'
};

const oldData = skuMap.get(superseded.old);
const newData = skuMap.get(superseded.new);

if (oldData && newData) {
    // Combine: add old consumption to new
    const combinedQty = oldData.totalQty + newData.totalQty;
    const combinedSpent = oldData.totalSpent + newData.totalSpent;
    const combinedLast = oldData.lastOrder > newData.lastOrder ? oldData.lastOrder : newData.lastOrder;
    const combinedOrders = oldData.orderCount + newData.orderCount;
    
    skuMap.set(superseded.new, {
        sku: superseded.new,
        description: `${newData.description} ← REPLACES ${superseded.old}`,
        category: newData.category,
        totalQty: combinedQty,
        totalSpent: combinedSpent,
        orderCount: combinedOrders,
        lastOrder: combinedLast
    });
    skuMap.delete(superseded.old);
}
...
for (const [sku, data] of skuMap.entries()) {
    ...
    // Round up to bundle size
    const bundleSize = 25;
    const suggestedQty = Math.ceil(quarterlyNeed / bundleSize) * bundleSize;

    results.push({
        SKU: sku,
        Description: data.description.length > 55 ? data.description.substring(0, 52) + '...' : data.description,
...
console.log("\n=== ULINE QUARTERLY ORDER (Consumption-Based) ===");
console.log(`⚠️  NOTE: S-4796 (22x14x6") REPLACES S-4905 (24x14x6")`);
console.log(`Report Period: ${reportStart.toLocaleDateString()} to ${reportEnd.toLocaleDateString()} | Quarterly = 90 days\n`);
...
for (const r of results) {
    const flag = r.SKU === superseded.new ? '⭐ ' : '  ';
    console.log(`${flag}${r.SKU.padEnd(8)} | ${r['Qtrly Need'].toString().padEnd(12)} | ${r['SUGGEST QTY'].toString().padEnd(14)} | ${r['Est. Cost'].padEnd(12)} | ${r['Last Order'].padEnd(8)} | ${r.Description}`);
...
console.log(`\n⭐ = REPLACES S-4905 (24x14x6")`);

// Save to CSV
const fs = require('fs');
const csvLines = ['SKU,Description,Unit_Cost,Daily_Vel,Qtrly_Need,Suggest_Qty,Est_Cost,Last_Order'];
for (const r of results) {
    csvLines.push(`${r.SKU},"${r.Description}",${r['Unit Cost']},${r['Daily Vel']},${r['Qtrly Need']},${r['SUGGEST QTY']},${r['Est. Cost'].replace('$', '')},${r['Last Order']}`);
}
fs.writeFileSync('uline-quarterly-order-adjusted.csv', csvLines.join('\n'));
console.log("\nCSV saved to uline-quarterly-order-adjusted.csv");
