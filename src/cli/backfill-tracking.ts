...
// Downloads all PDF attachments from a message and returns extracted text.
// Uses pdf-parse (no LLM). Falls back silently on parse errors.
async function extractAttachmentText(gmail: any, msgId: string, payload: any): Promise<string> {
    const texts: string[] = [];
    ...
    function walkParts(parts: any[]) {
        for (const p of parts ?? []) {
            ...
            if (isPDF && p.body?.attachmentId) {
                texts.push(p.body.attachmentId + "|" + (p.filename || ""));
            }
    ...
    for (const entry of texts) {
        ...
        try {
            ...
            // Encrypted or corrupt PDF — skip silently
        }
    }
...
async function main() {
    ...
    for (const threadId of threadIds) {
        ...
        for (const msg of thread.messages) {
            ...
            // Also extract text from any PDF attachments (BOL, delivery tickets, etc.)
            const pdfText = await extractAttachmentText(gmail, msg.id!, msg.payload);
            const combined = bodyText + "\n" + pdfText;
            const extracted = extractTracking(combined, msg.snippet || "", subject);
    ...
    for (const m of freightSearch?.messages ?? []) {
        ...
        const pdfText = await extractAttachmentText(gmail, msg.id!, msg.payload);
        const combined = bodyText + "\n" + pdfText;
        const allTracking = extractTracking(combined, msg.snippet || "", subject);
        if (allTracking.length === 0) continue;
