 ...
 */
export interface TrackingFoundResult {
    poNumber: string;
        ...
        try {
            ...
            for (const m of messages) {
                ...
                if (extractedTracking.length > 0) {
                    ...
                    let latestStatusCategory: string | null = null;
                    for (const tracking of extractedTracking) {
                        ...
                        if (record) {
                            ...
                            if (record.status_category) {
                                const cat = record.status_category;
                                if (cat === 'delivered' || (cat === 'shipped' && latestStatusCategory !== 'delivered')) {
                                    latestStatusCategory = cat;
                                } else if (!latestStatusCategory) {
                                    latestStatusCategory = cat;
                                }
                            }
                        }
                    ...
                    if (newTracking.length > 0) {
                        ...
                        foundTracking.push({ poNumber, trackingNumbers: deduped, statusCategory: latestStatusCategory });
                        console.log(`✅ [TrackingAgent] Found new tracking for PO #${poNumber}: ${newTracking.join(', ')} [${latestStatusCategory || 'unknown'}]`);
                    }
                }
            }
