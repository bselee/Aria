...
import fs from 'fs';
import { getTrackingStatus, carrierUrl, detectCarrier as detectCarrierSlug } from '../carriers/tracking-service';
...
// REFACTOR(2026-03-19): Removed duplicated LTL_DIRECT_LINKS, carrierUrl(), and
// detectCarrier() — now imported from ../carriers/tracking-service.ts

/**
 * Map tracking-service's lowercase slug to display name for the OOS report.
 */
function detectCarrier(trackingNumber: string): string {
    if (trackingNumber.includes(':::')) return trackingNumber.split(':::')[0];
    const slug = detectCarrierSlug(trackingNumber);
    if (slug === 'ups') return 'UPS';
    if (slug === 'usps') return 'USPS';
    if (slug === 'dhl') return 'DHL';
    if (slug === 'fedex') return 'FedEx';
    return 'Unknown';
}
...
export async function generateOOSExcel(
    ...
    titleRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF0D7377' } };
    titleRow.height = 28;

    ws.addRow(['Source: Stockie Low Stock Alert (dev@plutonian.io) | Generated by ARIA']);
    ws.mergeCells('A2:T2');
    const subtitleRow = ws.getRow(2);
    subtitleRow.font = { size: 10, bold: true, color: { argb: 'FFE8F4F4' } };
    subtitleRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF0A5C60' } };
    ...
    headerRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF14A085' } };
    ...
    for (const item of items) {
        ...
        if (item.openPOs.length === 0) {
            ...
            for (const po of item.openPOs) {
                ...
                poCell.font = { color: { argb: 'FF0D7377' }, underline: true };
                ...
                if (po.trackingLinks.length > 0) {
                    ...
                    trackCell.font = { color: { argb: 'FF0D7377' }, underline: true };
                }
            }
        }
    }
    ...
    const thinBorder: Partial<ExcelJS.Borders> = {
        top: { style: 'thin', color: { argb: 'FFCCE8E8' } },
        bottom: { style: 'thin', color: { argb: 'FFCCE8E8' } },
        left: { style: 'thin', color: { argb: 'FFCCE8E8' } },
        right: { style: 'thin', color: { argb: 'FFCCE8E8' } },
    };
    const stripeLight: ExcelJS.Fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF9FEFE' } };
    ...
    for (let r = 5; r <= ws.rowCount; r++) {
        ...
        if (actionVal.includes('NEEDS ORDER')) {
            ...
            cell.font = { bold: true, color: { argb: 'FFD97706' }, size: 10 };
            ...
            row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFFBF0' } };
        } else if (actionVal.includes('Shipped') || actionVal.includes('On order')) {
            cell.font = { color: { argb: 'FF0D7377' }, size: 10 };
            row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF0FAFA' } };
        } else if (actionVal.includes('Received')) {
            cell.font = { color: { argb: 'FF059669' }, size: 10 };
            row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF0FDF4' } };
        } else if (actionVal.includes('build')) {
            cell.font = { color: { argb: 'FF2563EB' }, size: 10 };
    ...
    sTitleRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF0D7377' } };
    sTitleRow.height = 28;
    ...
    sHdrRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF14A085' } };
    for (let c = 1; c <= 4; c++) {
    ...
    poTitleRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF0D7377' } };
    ...
    poHdrRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF14A085' } };
