...
export async function GET(req: Request) {
    try {
        ...
        const now = new Date();
        const end = now.toLocaleDateString('en-CA', { timeZone: 'America/Denver' });

        const start = new Date(now);
        start.setDate(start.getDate() - days);
        const startStr = start.toLocaleDateString('en-CA', { timeZone: 'America/Denver' });

        // endDate needs to be tomorrow so today's receivings are included
        ...
        return NextResponse.json({ received, days, asOf: end });
    } catch (err: any) {
