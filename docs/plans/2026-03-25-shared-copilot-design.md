...
Rolling summary creation is owned by the shared copilot core, not by channel adapters.

- The core performs a preflight token estimate before the main model call.
- If the request fits the budget, no summary call occurs.
- If the request exceeds the budget:
  - first try to reuse the latest stored thread summary, if it already covers all but the newest `4` turns
  - otherwise make one synchronous summarization call before the main turn
- The synchronous summary call blocks the current turn only when no usable cached summary exists.
- After the main turn completes, refresh the stored rolling summary asynchronously if the thread has advanced by `4+` turns beyond the cached summary.

This keeps current-turn behavior deterministic under pressure:

- cached summary available -> use it immediately
- no cached summary and over budget -> summarize now, then answer

The summary should be generated by the cheapest acceptable summarization path, not the full tool-calling loop.
...
### Read Tool Misrouting

Read-side tool failures are expected to be the most common copilot failure mode. The core should therefore treat read misrouting as recoverable:

- first read-tool result returns `no_result` or obvious mismatch -> the core may retry once with a different read tool
- second mismatch -> stop and return a concise failure message instead of thrashing
- do not convert read ambiguity into write behavior

This gives the system one recovery chance without hiding repeated confusion.
...
- any phase that refactors Telegram or dashboard adapters must verify Slack watchdog startup is unaffected before merge
